...
namespace eXl
{
  ...
  void LuaConsole::AddLog(const char* fmt, ...) IM_FMTARGS(2)
  {
    ...
    va_end(args);
    Items.push_back(AString(buf));
    ScrollToBottom = true;
  }

  void LuaConsole::Draw(/*const char* title, bool* p_open*/)
  {
...
#ifdef EXL_LUA
    //LuaManager::DoString(command_line, outResult);
#endif
    AddLog("%s", outResult.c_str());
  ...
  int LuaConsole::TextEditCallbackStub(ImGuiInputTextCallbackData* data) // In C++11 you are better off using lambdas for this sort of forwarding callbacks
  {

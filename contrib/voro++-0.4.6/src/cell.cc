...
{
  mv = 0;
  qu_size = 0 /*3 * (3 + hxy + hz*(hx + hy)))*/;
  qu = NULL /*(new int[qu_size])*/;
  qu_l = NULL /*(qu + qu_size)*/;
  mask = NULL /*new unsigned int[hxyz]*/;
  mask_size = 0;
}
...
void compute_data::init(unsigned int hx, unsigned int hy, unsigned int hz, unsigned int hxy, unsigned int hxyz)
{
  //mv = 0;
  if (qu_size < 3 * (3 + hxy + hz*(hx + hy)))
  {

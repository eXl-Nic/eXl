...
void container_base::update_box_size(double iMinX, double iMaxX, double iMinY, double iMaxY, double iMinZ, double iMaxZ)
{
  ax = iMinX;
  bx = iMaxX;
  ay = iMinY;
  ...
  az = iMinZ;
  bz = iMaxZ;
  voro_base::update_box_size((bx - ax) / nx, (by - ay) / ny, (bz - az) / nz);
}
...
bool container_poly::find_voronoi_cell(double x, double y, double z, double &rx, double &ry, double &rz, int &pid) 
{
  ...
  return find_voronoi_cell(tempData, x, y, z, rx, ry, rz, pid);
}

bool container_poly::find_voronoi_cell(compute_data& iData, double x, double y, double z, double &rx, double &ry, double &rz, int &pid) {
	int ai,aj,ak,ci,cj,ck,ijk;
	particle_record w;
	double mrs;
	...
	// is periodic, then remap it back into the domain
	if(!remap(ai,aj,ak,ci,cj,ck,x,y,z,ijk)) return false;
	vc.find_voronoi_cell(iData, x,y,z,ci,cj,ck,ijk,w,mrs);

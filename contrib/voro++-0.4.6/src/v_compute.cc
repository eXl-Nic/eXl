...
template<class c_class>
inline void voro_compute<c_class>::scan_all(typename c_class::RadiusData const& rData, int ijk, double x, double y, double z, int di, int dj, int dk, particle_record &w, double &mrs) const{
	double x1,y1,z1,rs;bool in_block=false;
	for(int l=0;l<co[ijk];l++) {
		x1=p[ijk][ps*l]-x;
		y1=p[ijk][ps*l+1]-y;
		z1=p[ijk][ps*l+2]-z;
		rs=rData.r_current_sub(x1*x1+y1*y1+z1*z1,ijk,l);
...
template<class c_class>
void voro_compute<c_class>::find_voronoi_cell(compute_data& iData, double x,double y,double z,int ci,int cj,int ck,int ijk,particle_record &w,double &mrs) const {
	double qx=0,qy=0,qz=0,rs;
	int i,j,k,di,dj,dk,ei,ej,ek,f,g,disp;
	double fx,fy,fz,mxs,mys,mzs,*radp;
	unsigned int q,*e,*mijk;

  typename c_class::RadiusData rData(con);
  
  iData.init(hx, hy, hz, hxy, hxyz);
	...
	scan_all(rData,ijk,x,y,z,0,0,0,w,mrs);
	...
	do {
		...
		if(rData.r_max_add(mrs)<radp[g]) return;
		g++;
		...
		// returns it in crs.
		if(compute_min_radius(rData,di,dj,dk,fx,fy,fz,mrs)) continue;
		...
		scan_all(rData,ijk,x-qx,y-qy,z-qz,di,dj,dk,w,mrs);
	} while(g<f);

	// Update mask value and initialize queue
	iData.mv++;
  if (iData.mv == 0) { reset_mask(iData); iData.mv = 1; }
  int *qu_s = iData.qu, *qu_e = iData.qu;
	...
	while(g<wl_seq_length-1) {
		...
		ej=dj+j;if(ej<0||ej>=hy) continue;
		ek=dk+k;if(ek<0||ek>=hz) continue;
    mijk = iData.mask + ei + hx*(ej + hy*ek);
    *mijk = iData.mv;
    ...
    if (qu_e>iData.qu_l - 18) add_list_memory(iData, qu_s, qu_e);
		scan_bits_mask_add(iData, q,mijk,ei,ej,ek,qu_e);
	}
    ...
    if (qu_s == iData.qu_l) qu_s = iData.qu;
    ...
    if ((qu_s <= qu_e ? (iData.qu_l - qu_e) + (qu_s - iData.qu) : qu_s - qu_e)<18) add_list_memory(iData,qu_s, qu_e);
		add_to_mask(iData, ei,ej,ek,qu_e);
	}
}
...
template<class c_class>
inline void voro_compute<c_class>::add_to_mask(compute_data& iData, int ei, int ej, int ek, int *&qu_e) const{
	unsigned int *mijk=iData.mask+ei+hx*(ej+hy*ek);
  if (ek>0) if (*(mijk - hxy) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk - hxy) = iData.mv; *(qu_e++) = ei; *(qu_e++) = ej; *(qu_e++) = ek - 1; }
  if (ej>0) if (*(mijk - hx) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk - hx) = iData.mv; *(qu_e++) = ei; *(qu_e++) = ej - 1; *(qu_e++) = ek; }
  if (ei>0) if (*(mijk - 1) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk - 1) = iData.mv; *(qu_e++) = ei - 1; *(qu_e++) = ej; *(qu_e++) = ek; }
  if (ei<hx - 1) if (*(mijk + 1) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk + 1) = iData.mv; *(qu_e++) = ei + 1; *(qu_e++) = ej; *(qu_e++) = ek; }
  if (ej<hy - 1) if (*(mijk + hx) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk + hx) = iData.mv; *(qu_e++) = ei; *(qu_e++) = ej + 1; *(qu_e++) = ek; }
  if (ek<hz - 1) if (*(mijk + hxy) != iData.mv) { if (qu_e == iData.qu_l) qu_e = iData.qu; *(mijk + hxy) = iData.mv; *(qu_e++) = ei; *(qu_e++) = ej; *(qu_e++) = ek + 1; }
}
 ...
 * \param[in,out] qu_e a pointer to the end of the queue. */
template<class c_class>
inline void voro_compute<c_class>::scan_bits_mask_add(compute_data& iData, unsigned int q, unsigned int *mijk, int ei, int ej, int ek, int *&qu_e) const{
	const unsigned int b1=1<<21,b2=1<<22,b3=1<<24,b4=1<<25,b5=1<<27,b6=1<<28;
  unsigned int const mv = iData.mv;
	if((q&b2)==b2) {
...
bool voro_compute<c_class>::compute_cell(v_cell &c,int ijk,int s,int ci,int cj,int ck) const {
  ...
  compute_data& cData = c.get_compute_d();
  cData.init(hx,hy,hz,hxy,hxyz);

	if(!con.initialize_voronoicell(c,ijk,s,ci,cj,ck,i,j,k,x,y,z,disp)) return false;

  typename c_class::RadiusData rData(con);
	con.r_init(rData,ijk,s);
	...
	for(l=0;l<s;l++) {
		...
		y1=p[ijk][ps*l+1]-y;
		z1=p[ijk][ps*l+2]-z;
		rs=rData.r_scale(x1*x1+y1*y1+z1*z1,ijk,l);
	...
	do {
		...
		// If mrs is less than the minimum distance to any untested
		// block, then we are done
		if(rData.r_ctest(radp[g],mrs)) return true;
		g++;
		...
		// Otherwise, it computes the maximum distance to the block and
		// returns it in crs.
		if(compute_min_max_radius(rData,di,dj,dk,fx,fy,fz,gxs,gys,gzs,crs,mrs)) continue;
		...
		if(co[ijk]>0) {
			...
			if(!rData.r_ctest(crs,mrs)) {
				...
				do {
					...
					if(rData.r_scale_check(rs,mrs,ijk,l)&&!c.nplane(x1,y1,z1,rs,id[ijk][l])) return false;
					l++;
				} while (l<co[ijk]);
			}
		}
  ...
  cData.mv++;
  if (cData.mv == 0) { reset_mask(cData); cData.mv = 1; }

	// Set the queue pointers
  int *qu_s = cData.qu, *qu_e = cData.qu;
	...
	while(g<wl_seq_length-1) {
		...
		ej=dj+j;if(ej<0||ej>=hy) continue;
		ek=dk+k;if(ek<0||ek>=hz) continue;
    mijk = cData.mask + ei + hx*(ej + hy*ek);
    *mijk = cData.mv;
    ...
    if (qu_e>cData.qu_l - 18) add_list_memory(cData, qu_s, qu_e);
    ...
    scan_bits_mask_add(cData, q, mijk, ei, ej, ek, qu_e);
	}
	...
	while(qu_s!=qu_e) {
		...
		// start
    if (qu_s == cData.qu_l) qu_s = cData.qu;
		...
		if(ei>i) {
			if(ej>j) {
				if(ek>k) {if(corner_test(rData,c,xlo,ylo,zlo,xhi,yhi,zhi)) continue;}
        else if (ek<k) { if (corner_test(rData, c, xlo, ylo, zhi, xhi, yhi, zlo)) continue; }
        else { if (edge_z_test(rData, c, xlo, ylo, zlo, xhi, yhi, zhi)) continue; }
			} else if(ej<j) {
        if (ek>k) { if (corner_test(rData, c, xlo, yhi, zlo, xhi, ylo, zhi)) continue; }
        else if (ek<k) { if (corner_test(rData, c, xlo, yhi, zhi, xhi, ylo, zlo)) continue; }
        else { if (edge_z_test(rData, c, xlo, yhi, zlo, xhi, ylo, zhi)) continue; }
			} else {
        if (ek>k) { if (edge_y_test(rData, c, xlo, ylo, zlo, xhi, yhi, zhi)) continue; }
        else if (ek<k) { if (edge_y_test(rData, c, xlo, ylo, zhi, xhi, yhi, zlo)) continue; }
        else { if (face_x_test(rData, c, xlo, ylo, zlo, yhi, zhi)) continue; }
			}
		} else if(ei<i) {
			if(ej>j) {
        if (ek>k) { if (corner_test(rData, c, xhi, ylo, zlo, xlo, yhi, zhi)) continue; }
        else if (ek<k) { if (corner_test(rData, c, xhi, ylo, zhi, xlo, yhi, zlo)) continue; }
        else { if (edge_z_test(rData, c, xhi, ylo, zlo, xlo, yhi, zhi)) continue; }
			} else if(ej<j) {
        if (ek>k) { if (corner_test(rData, c, xhi, yhi, zlo, xlo, ylo, zhi)) continue; }
        else if (ek<k) { if (corner_test(rData, c, xhi, yhi, zhi, xlo, ylo, zlo)) continue; }
        else { if (edge_z_test(rData, c, xhi, yhi, zlo, xlo, ylo, zhi)) continue; }
			} else {
        if (ek>k) { if (edge_y_test(rData, c, xhi, ylo, zlo, xlo, yhi, zhi)) continue; }
        else if (ek<k) { if (edge_y_test(rData, c, xhi, ylo, zhi, xlo, yhi, zlo)) continue; }
        else { if (face_x_test(rData, c, xhi, ylo, zlo, yhi, zhi)) continue; }
			}
		} else {
			if(ej>j) {
        if (ek>k) { if (edge_x_test(rData, c, xlo, ylo, zlo, xhi, yhi, zhi)) continue; }
        else if (ek<k) { if (edge_x_test(rData, c, xlo, ylo, zhi, xhi, yhi, zlo)) continue; }
        else { if (face_y_test(rData, c, xlo, ylo, zlo, xhi, zhi)) continue; }
			} else if(ej<j) {
        if (ek>k) { if (edge_x_test(rData, c, xlo, yhi, zlo, xhi, ylo, zhi)) continue; }
        else if (ek<k) { if (edge_x_test(rData, c, xlo, yhi, zhi, xhi, ylo, zlo)) continue; }
        else { if (face_y_test(rData, c, xlo, yhi, zlo, xhi, zhi)) continue; }
			} else {
        if (ek>k) { if (face_z_test(rData, c, xlo, ylo, zlo, xhi, yhi)) continue; }
        else if (ek<k) { if (face_z_test(rData, c, xlo, ylo, zhi, xhi, yhi)) continue; }
    ...
    if ((qu_s <= qu_e ? (cData.qu_l - qu_e) + (qu_s - cData.qu) : qu_s - qu_e)<18) add_list_memory(cData, qu_s, qu_e);

		// Test the neighbors of the current block, and add them to the
		// block list if they haven't already been tested
		add_to_mask(cData, ei,ej,ek,qu_e);
	}
...
template<class c_class>
template<class v_cell>
bool voro_compute<c_class>::corner_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double yl, double zl, double xh, double yh, double zh) const{
	rData.r_prime(xl*xl+yl*yl+zl*zl);
	if(c.plane_intersects_guess(xh,yl,zl,rData.r_cutoff(xl*xh+yl*yl+zl*zl))) return false;
	if(c.plane_intersects(xh,yh,zl,rData.r_cutoff(xl*xh+yl*yh+zl*zl))) return false;
	if(c.plane_intersects(xl,yh,zl,rData.r_cutoff(xl*xl+yl*yh+zl*zl))) return false;
	if(c.plane_intersects(xl,yh,zh,rData.r_cutoff(xl*xl+yl*yh+zl*zh))) return false;
	if(c.plane_intersects(xl,yl,zh,rData.r_cutoff(xl*xl+yl*yl+zl*zh))) return false;
	if(c.plane_intersects(xh,yl,zh,rData.r_cutoff(xl*xh+yl*yl+zl*zh))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::edge_x_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double yl, double zl, double x1, double yh, double zh) const{
	rData.r_prime(yl*yl+zl*zl);
	if(c.plane_intersects_guess(x0,yl,zh,rData.r_cutoff(yl*yl+zl*zh))) return false;
	if(c.plane_intersects(x1,yl,zh,rData.r_cutoff(yl*yl+zl*zh))) return false;
	if(c.plane_intersects(x1,yl,zl,rData.r_cutoff(yl*yl+zl*zl))) return false;
	if(c.plane_intersects(x0,yl,zl,rData.r_cutoff(yl*yl+zl*zl))) return false;
	if(c.plane_intersects(x0,yh,zl,rData.r_cutoff(yl*yh+zl*zl))) return false;
	if(c.plane_intersects(x1,yh,zl,rData.r_cutoff(yl*yh+zl*zl))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::edge_y_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double y0, double zl, double xh, double y1, double zh) const{
	rData.r_prime(xl*xl+zl*zl);
	if(c.plane_intersects_guess(xl,y0,zh,rData.r_cutoff(xl*xl+zl*zh))) return false;
	if(c.plane_intersects(xl,y1,zh,rData.r_cutoff(xl*xl+zl*zh))) return false;
	if(c.plane_intersects(xl,y1,zl,rData.r_cutoff(xl*xl+zl*zl))) return false;
	if(c.plane_intersects(xl,y0,zl,rData.r_cutoff(xl*xl+zl*zl))) return false;
	if(c.plane_intersects(xh,y0,zl,rData.r_cutoff(xl*xh+zl*zl))) return false;
	if(c.plane_intersects(xh,y1,zl,rData.r_cutoff(xl*xh+zl*zl))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::edge_z_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double yl, double z0, double xh, double yh, double z1) const{
	rData.r_prime(xl*xl+yl*yl);
	if(c.plane_intersects_guess(xl,yh,z0,rData.r_cutoff(xl*xl+yl*yh))) return false;
	if(c.plane_intersects(xl,yh,z1,rData.r_cutoff(xl*xl+yl*yh))) return false;
	if(c.plane_intersects(xl,yl,z1,rData.r_cutoff(xl*xl+yl*yl))) return false;
	if(c.plane_intersects(xl,yl,z0,rData.r_cutoff(xl*xl+yl*yl))) return false;
	if(c.plane_intersects(xh,yl,z0,rData.r_cutoff(xl*xh+yl*yl))) return false;
	if(c.plane_intersects(xh,yl,z1,rData.r_cutoff(xl*xh+yl*yl))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::face_x_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double y0, double z0, double y1, double z1) const{
	rData.r_prime(xl*xl);
	if(c.plane_intersects_guess(xl,y0,z0,rData.r_cutoff(xl*xl))) return false;
	if(c.plane_intersects(xl,y0,z1,rData.r_cutoff(xl*xl))) return false;
	if(c.plane_intersects(xl,y1,z1,rData.r_cutoff(xl*xl))) return false;
	if(c.plane_intersects(xl,y1,z0,rData.r_cutoff(xl*xl))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::face_y_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double yl, double z0, double x1, double z1) const{
	rData.r_prime(yl*yl);
	if(c.plane_intersects_guess(x0,yl,z0,rData.r_cutoff(yl*yl))) return false;
	if(c.plane_intersects(x0,yl,z1,rData.r_cutoff(yl*yl))) return false;
	if(c.plane_intersects(x1,yl,z1,rData.r_cutoff(yl*yl))) return false;
	if(c.plane_intersects(x1,yl,z0,rData.r_cutoff(yl*yl))) return false;
	return true;
}
...
template<class c_class>
template<class v_cell>
inline bool voro_compute<c_class>::face_z_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double y0, double zl, double x1, double y1) const{
	rData.r_prime(zl*zl);
	if(c.plane_intersects_guess(x0,y0,zl,rData.r_cutoff(zl*zl))) return false;
	if(c.plane_intersects(x0,y1,zl,rData.r_cutoff(zl*zl))) return false;
	if(c.plane_intersects(x1,y1,zl,rData.r_cutoff(zl*zl))) return false;
	if(c.plane_intersects(x1,y0,zl,rData.r_cutoff(zl*zl))) return false;
	return true;
}
 ...
 *         within mrs. */
template<class c_class>
bool voro_compute<c_class>::compute_min_max_radius(typename c_class::RadiusData const& rData, int di,int dj,int dk,double fx,double fy,double fz,double gxs,double gys,double gzs,double &crs,double mrs) const{
	double xlo,ylo,zlo;
	if(di>0) {
		xlo=di*boxx-fx;
		crs=xlo*xlo;
		if(dj>0) {
			...
			if(dk>0) {
				zlo=dk*boxz-fz;
				crs+=zlo*zlo;if(rData.r_ctest(crs,mrs)) return true;
				...
				if(rData.r_ctest(crs,mrs)) return true;
		...
		if(dj>0) {
			...
			if(dk>0) {
				zlo=dk*boxz-fz;crs=zlo*zlo;if(rData.r_ctest(crs,mrs)) return true;
				crs+=boxz*(2*zlo+boxz);
			} else if(dk<0) {
				zlo=(dk+1)*boxz-fz;crs=zlo*zlo;if(rData.r_ctest(crs,mrs)) return true;
				crs+=boxz*(-2*zlo+boxz);
			} else {
				crs=0;
...
template<class c_class>
bool voro_compute<c_class>::compute_min_radius(typename c_class::RadiusData const& rData, int di, int dj, int dk, double fx, double fy, double fz, double mrs) const{
	double t,crs;
	...
	return crs>rData.r_max_add(mrs);
}
...
template<class c_class>
inline void voro_compute<c_class>::add_list_memory(compute_data& iData, int*& qu_s, int*& qu_e) const {
	iData.qu_size<<=1;
	int *qu_n=new int[iData.qu_size],*qu_c=qu_n;
#if VOROPP_VERBOSE >=2
    ...
    while (qu_s<iData.qu_l) *(qu_c++) = *(qu_s++); qu_s = iData.qu;
		while(qu_s<qu_e) *(qu_c++)=*(qu_s++);
	}
  delete[] iData.qu;
  qu_s = iData.qu = qu_n;
  iData.qu_l = iData.qu + iData.qu_size;
	qu_e=qu_c;
}
...
template bool voro_compute<container>::compute_cell(voronoicell&,int,int,int,int,int) const;
template bool voro_compute<container>::compute_cell(voronoicell_neighbor&,int,int,int,int,int) const;
template void voro_compute<container>::find_voronoi_cell(double,double,double,int,int,int,int,particle_record&,double&) const;
template bool voro_compute<container_poly>::compute_cell(voronoicell&,int,int,int,int,int) const;
template bool voro_compute<container_poly>::compute_cell(voronoicell_neighbor&,int,int,int,int,int) const;
template void voro_compute<container_poly>::find_voronoi_cell(double,double,double,int,int,int,int,particle_record&,double&) const;
...
//template bool voro_compute<container_periodic>::compute_cell(voronoicell&,int,int,int,int,int) const;
//template bool voro_compute<container_periodic>::compute_cell(voronoicell_neighbor&,int,int,int,int,int) const;
//template void voro_compute<container_periodic>::find_voronoi_cell(double,double,double,int,int,int,int,particle_record&,double&) const;
//template bool voro_compute<container_periodic_poly>::compute_cell(voronoicell&,int,int,int,int,int) const;
//template bool voro_compute<container_periodic_poly>::compute_cell(voronoicell_neighbor&,int,int,int,int,int) const;
//template void voro_compute<container_periodic_poly>::find_voronoi_cell(double,double,double,int,int,int,int,particle_record&,double&) const;

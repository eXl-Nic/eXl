...
class voro_compute {
	public:
		...
		bool compute_cell(v_cell &c,int ijk,int s,int ci,int cj,int ck) const;
    inline void find_voronoi_cell(double x, double y, double z, int ci, int cj, int ck, int ijk, particle_record &w, double &mrs) const
    {
      compute_data tempData;
      find_voronoi_cell(tempData, x, y, z, ci, cj, ck, ijk, w, mrs);
    }
    void find_voronoi_cell(compute_data& iData, double x, double y, double z, int ci, int cj, int ck, int ijk, particle_record &w, double &mrs) const;
	private:
		...
		//int *qu_l;
		template<class v_cell>
    bool corner_test(typename c_class::RadiusData & rData, v_cell &c, double xl, double yl, double zl, double xh, double yh, double zh) const;
		template<class v_cell>
    inline bool edge_x_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double yl, double zl, double x1, double yh, double zh) const;
		template<class v_cell>
    inline bool edge_y_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double y0, double zl, double xh, double y1, double zh) const;
		template<class v_cell>
    inline bool edge_z_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double yl, double z0, double xh, double yh, double z1) const;
		template<class v_cell>
    inline bool face_x_test(typename c_class::RadiusData& rData, v_cell &c, double xl, double y0, double z0, double y1, double z1) const;
		template<class v_cell>
    inline bool face_y_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double yl, double z0, double x1, double z1) const;
		template<class v_cell>
    inline bool face_z_test(typename c_class::RadiusData& rData, v_cell &c, double x0, double y0, double zl, double x1, double y1) const;
    bool compute_min_max_radius(typename c_class::RadiusData const& rData, int di, int dj, int dk, double fx, double fy, double fz, double gx, double gy, double gz, double& crs, double mrs) const;
    bool compute_min_radius(typename c_class::RadiusData const& rData, int di, int dj, int dk, double fx, double fy, double fz, double mrs) const;
    inline void add_to_mask(compute_data& iData, int ei, int ej, int ek, int *&qu_e) const;
    inline void scan_bits_mask_add(compute_data& iData, unsigned int q, unsigned int *mijk, int ei, int ej, int ek, int *&qu_e) const;
    inline void scan_all(typename c_class::RadiusData const& rData, int ijk, double x, double y, double z, int di, int dj, int dk, particle_record &w, double &mrs) const;
    void add_list_memory(compute_data& iData, int*& qu_s, int*& qu_e) const;
		/** Resets the mask in cases where the mask counter wraps
		 * around. */
		inline void reset_mask(compute_data& iData) const 
    {
      for (unsigned int *mp(iData.mask); mp < iData.mask + hxyz; mp++)
      {
        *mp = 0;
      }
		}
